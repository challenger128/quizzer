...
@result_router.post('/', summary="Create a result", response_model=ResultOut)
...
@result_router.get('/{result.id}', summary="Get a result yby its id", response_model=QuizResult)
async def get_result(result_id: UUID, current_user: User = Depends(get_current_user)):
    return await ResultService.retrieve_result(result_id, current_user)
